...
export class BoardStandard implements Board {
  ...
  private _getSizeOfColumn(column: ("R" | "Y" | "-")[]) {
  ...
  public applyMove(
    ...
    if (redWin && yellowWin) return new BoardStandard(false, newSlots, "DRAW");
    if (redWin) return new BoardStandard(false, newSlots, "RED");
    if (yellowWin) return new BoardStandard(false, newSlots, "YELLOW");
    ...
    if (usedSlots === this.COLUMNS * this.ROWS)
      return new BoardStandard(false, newSlots, "DRAW");

    // If not invalid, no winners and no draws, then everything is OK
    return new BoardStandard(false, newSlots);
  }
}
